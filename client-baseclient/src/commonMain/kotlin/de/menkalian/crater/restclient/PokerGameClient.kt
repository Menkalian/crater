...
import de.menkalian.crater.currentSystemTimeMillis
import de.menkalian.crater.data.Draco
import de.menkalian.crater.data.game.Player
import de.menkalian.crater.data.game.PokerLobby
import de.menkalian.crater.data.game.SocketPackage
import de.menkalian.crater.data.game.enums.AnswerRevealStrategy
import de.menkalian.crater.data.game.enums.BlindRaiseStrategy
import de.menkalian.crater.data.game.enums.ConnectionState
import de.menkalian.crater.data.game.enums.GameState
import de.menkalian.crater.data.game.enums.LateJoinBehaviour
import de.menkalian.crater.data.game.enums.LobbyPublicity
import de.menkalian.crater.data.game.enums.PackageType
import de.menkalian.crater.data.game.enums.PlayerRole
import de.menkalian.crater.data.game.enums.TimeoutStrategy
import de.menkalian.crater.data.game.values.RegexValueMultiChangeListener
import de.menkalian.crater.data.game.values.TransferableValue
import de.menkalian.crater.data.game.values.Values
import de.menkalian.crater.data.task.Category
import de.menkalian.crater.data.task.Difficulty
...
class PokerGameClient
    ...
    private val configuration: CraterClientConfiguration
) {
    companion object {
        var msgIdCounter = 0L
                ...
                Draco.Game.Poker.Question.UUID   -> {
                    try {
                        ...
                        lobby.gameStateValues.currentQuestion = lobby.gameStateValues.currentQuestion?.copy(id = id)
                            ?: Task(id, "", 0L, Language.ENGLISH, Difficulty.EASY, Category.ART, "", 0.0, "", listOf())
                ...
                Draco.Game.Poker.Question.Text   -> {
                    try {
                        lobby.gameStateValues.currentQuestion = lobby.gameStateValues.currentQuestion?.copy(question = value.toString())
                            ?: Task(0, "", 0L, Language.ENGLISH, Difficulty.EASY, Category.ART, value.toString(), 0.0, "", listOf())
                    } catch (ex: Exception) {
                        lobby.gameStateValues.currentQuestion = null
                    }
                }
                Draco.Game.Poker.Question.Answer -> {
                    try {
                        ...
                        lobby.gameStateValues.currentQuestion = lobby.gameStateValues.currentQuestion?.copy(answer = answer)
                            ?: Task(0, "", 0L, Language.ENGLISH, Difficulty.EASY, Category.ART, "", answer, "", listOf())
                    } catch (ex: Exception) {
                        lobby.gameStateValues.currentQuestion = null
                    }
                }
